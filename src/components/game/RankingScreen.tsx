  ...
  return (
    <ul className="flex flex-col gap-2 mt-4">
      {rows.map((r, i) => {
        ...
        const medal = i === 0 ? <Crown className="h-5 w-5 text-yellow-400" />
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold truncate">{r.display_name}{isMe && " (tu)"}</div>
  ...
  return (
    <div className="min-h-screen pb-32 p-5 flex flex-col gap-5 animate-fade-in">
      <header className="flex items-center gap-3">
        <div className="h-12 w-12 rounded-2xl bg-gradient-treasure flex items-center justify-center shadow-glow">
          <Trophy className="h-6 w-6 text-background" />
        </div>
        <div className="flex-1">
          <h2 className="text-2xl font-black leading-tight">Ranking</h2>
          <p className="text-xs text-muted-foreground">Mais pontos · menos tempo</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => go("home")} className="rounded-full">Voltar</Button>
      </header>
